 ...
 * reason: the token this application holds opens other people's records. It
 * renders on the server, holds the session, and the browser receives nothing it
 * could replay.
 *
...
const config = {
  ...
  experimental: {
    ...
    serverActions: {
      allowedOrigins: (process.env.GOVERNMENT_PORTAL_ALLOWED_ORIGINS ?? '')
        .split(',')
        .filter(Boolean),
    },
  },
