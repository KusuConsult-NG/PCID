/**
 * Enough of an API client to let a second officer act during a portal test.
 *
 * This portal's subject is what one officer may do that another may not, and
 * several of those assertions need a precondition only somebody else can
 * create: a supervisor assigned to the case, an access request waiting for a
 * decision. Those are driven through the real API as the real second officer
 * rather than by writing rows behind the platform's back, so a test cannot pass
 * against a state the authorisation rules would never have allowed.
 *
...
/**
 * Wait until the next thirty-second step begins.
 *
 * The platform refuses a time step at or below the last one presented, which is
 * correct - a code should not work twice - and awkward for a suite whose seed
 * command signed in with the same authenticator moments earlier. Only the tests
 * have to work around it.
 */
export async function waitForNextTimeStep(): Promise<void> {
  const now = Date.now();
  const nextBoundary = (Math.floor(now / 30_000) + 1) * 30_000;
  await new Promise((settle) => setTimeout(settle, nextBoundary - now + 1_000));
}
